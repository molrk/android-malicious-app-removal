      ...
      if (mainWindow) {
        ...
        if (code !== 0) {
          mainWindow.webContents.send(
            "logcat-error",
            "Device may have disconnected or ADB connection lost",
          );
        }
    ...
    logcatProcess.on("error", (error) => {
      ...
      if (mainWindow) {
        if (error.code === "ENOENT") {
          mainWindow.webContents.send(
            "logcat-error",
            "ADB command not found. Please check ADB installation.",
          );
        } else {
...
ipcMain.handle("check-adb", async () => {
  ...
  return {
    available: false,
    error:
      "ADB not found in common locations. Please add ADB to PATH or place adb.exe in the project folder.",
    ...
    if (mainWindow && global.adbPath) {
      try {
        ...
        if (logcatProcess && result.devices.length === 0) {
          mainWindow.webContents.send(
            "logcat-error",
            "Device disconnected - stopping logcat",
          );
          if (process.platform === "win32") {
            const { spawn } = require("child_process");
            spawn("taskkill", ["/pid", logcatProcess.pid, "/t", "/f"]);
